...
async function handleAnalysis(tabId, owner, repo) {
    try {
        let keyResolution;
        try {
        ...
        if (cached) {
            sendToTab(tabId, { type: 'ANALYSIS_COMPLETE', result: cached });
            return;
        }
        ...
        const result = await analyzeWithGemini(keyResolution.key, owner, repo, contextFiles);
